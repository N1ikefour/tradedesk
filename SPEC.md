...
4. Установка (`S1-10`): `run-collector.bat` — проверка версии Python, `.venv`, `pip install -e .[mt5]`, сборка `collector.env` и запуск менеджера; `install-service.ps1` — регистрация в Планировщике заданий «при входе в систему», перезапуск при сбое, явная рабочая папка; `install-service.bat`, `stop-collector.bat` и `status-collector.bat` — то же двойным кликом. Сценарии ручной проверки всего коллектора собраны в `docs/collector-windows-checklist.md`.
...
**`COLLECTOR_TOKEN` переносится в `collector.env` из `.env` установки** (решено в `S1-10`, `collector/bootstrap.py`). Человек иначе копирует 64 символа глазами, а ошибка в них выглядит как «TradeDesk не отвечает» через минуту работы. Значение при этом не печатается ни в консоль, ни в файл лога. Остальные поля образца остаются как есть: путь к терминалу и `MAX_ACCOUNTS` — решения человека, и первый запуск на них останавливается, чтобы он их принял.

**Остановка — `stop-collector.bat`, и это часть контракта, а не удобство** (решено в `S1-10`). Сначала менеджер **просят** выйти самому: скрипт кладёт рядом с `collector.env` файл `collector-stop.flag`, менеджер видит его своим тиком и уходит обычным путём — через `_shutdown`: сам гасит процессы счетов и шлёт за них `state=stopped`. Просьба именно файлом, потому что доставить сигнал чужому процессу на Windows нечем: `SIGTERM` там не межпроцессный, а `CTRL_BREAK` требует присоединения к консоли жертвы. Ждать бесконечно нельзя — тик с недоступным API тянется минутами, — поэтому потолок 20 секунд, а дальше гасится задача Планировщика, менеджер и оставшиеся процессы счетов, именно в этом порядке: живой менеджер поднимает процесс счёта заново своим следующим тиком. Процессы коллектора отбираются по пути `.venv\Scripts\python.exe`: у порождённого через `multiprocessing` процесса счёта слова `collector` в командной строке нет вовсе. Терминалы `terminal64.exe` при этом не закрываются — они перечисляются человеку с их путями (§8.3, допущение 36).

⚠️ **Мягкая остановка обещанием быть не может.** Успела она или нет, зависит от того, где менеджер был в момент просьбы, и проверить это на Windows пока некому. Поэтому скрипт печатает не обещание, а результат: успел — «менеджер попрощался», не успел — «остановлен принудительно», и тогда же говорит цену, потому что цена видна человеку не сразу. Принудительная остановка до `_shutdown` не доходит: прощальный `state=stopped` не уходит, а процессы счетов гасит уже сам скрипт. **Карточки счетов два исхода не различают, и обещать обратное скрипт не вправе** (`X-62`): `state=stopped` не меняет ни статуса, ни текста (§5.3), а состояния «коллектор остановлен» в перечне §9.3 нет вовсе — после любой остановки на карточках сразу не меняется ничего, а через пять минут `check_collectors` (§10) пишет там «коллектор не на связи», то есть вид поломки после осознанного действия. Разницу между исходами человек видит в окне скрипта и в диспетчере задач, и только там. Данные при этом не теряются: `last_sync_at` двигает только принятый батч, а следующий запуск забирает своё окно целиком.
...
**Периодический heartbeat шлёт менеджер, за все свои счета сразу** (решено в `S1-09`, уточняет пункт 1). Процесс счёта продолжает слать свой — по смене состояния (`S1-08`), — и они не спорят: `state=running` обновляет только `last_heartbeat_at`, а статус и текст причины ставит `state=error`, чей угодно (`accounts.service.apply_heartbeat`). Причина в том, что о счёте **без живого процесса** рассказать больше некому: сверх лимита, процесс упал, перезапуски кончились. Молчание такого счёта пять минут увело бы его в «коллектор не на связи» — симптом, уводящий от причины.

**Менеджер молчит о том, о чём процесс счёта сказал сам** (решено в `S1-09`). Код выхода 4 (`EXIT_ACCOUNT`) означает «я уже отправил heartbeat с точной причиной»: счёт не в USD, батч отвергнут, задания на счёт нет. Такому счёту менеджер шлёт `state=running` **без сообщения** — причина на карточке остаётся, а `last_heartbeat_at` двигается, и через пять минут `check_collectors` (§10) не заменит диагноз на «коллектор не на связи». Своё сообщение менеджер пишет только там, где сказать больше некому: процесс убит системой, `spawn` отказал, конфигурация или платформа (коды 2 и 3), — и когда причина неизвестна ему самому, он это так и называет, а не подставляет правдоподобный код.

⚠️ **Цена решения: `state=running` от менеджера значит «коллектор ведёт этот счёт», а не «процесс сейчас жив».** Живого процесса за этим heartbeat может не быть — он умер, объяснив причину, и ждёт перезапуска или уже исчерпал попытки. Единственное, что `running` утверждает, — что коллектор про этот счёт знает и не потерял его; всё остальное человек читает в `status_message`, поставленном тем, кто знал точнее.

**Место в пуле закрепляется за счётом, а не за живым процессом** (решено в `S1-09`). Счёт, чей процесс умер и ждёт перезапуска, места не теряет; порядок ответа assignments его тоже не отбирает. Иначе счёт сверх лимита занимал бы освободившееся место на время перезапуска, вернувшийся оказывался бы сверх лимита сам, и сообщение о превышении `MAX_ACCOUNTS` ходило бы по кругу — человек читал бы каждую минуту новую причину на новом счёте.

**Упавший процесс перезапускается пять раз подряд, дальше — отказ словами** (решено в `S1-09`). Пауза между попытками удваивается от пяти секунд: 5 → 10 → 20 → 40 → 80 с. Потолок в формуле — 15 минут, но при пяти попытках он недостижим, и политику описывает именно этот ряд. Процесс, проживший 10 минут, обнуляет счёт попыток. Коды выхода 2 (`collector.env`) и 3 (не Windows) не перезапускаются вовсе — у нового процесса будут те же настройки и та же ОС. Исчерпанные перезапуски и отказ по конфигурации видны на карточке счёта разными текстами: в первом случае человеку нужен файл лога, во втором — правка `collector.env`. Если же процесс каждый раз уходил с кодом 4, на карточке остаётся его собственная причина — она объясняет и почему он уходит, и что чинить, а «пришлите файл лога» на её месте было бы шагом назад.

⚠️ **Паузы квантуются тиком менеджера**: перезапуск случается раз в `HEARTBEAT_INTERVAL_SECONDS` (по умолчанию 60 с), поэтому 5, 10, 20 и 40 секунд от него неотличимы — процесс, умерший на 60-й секунде, поднимется на 120-й. В логе это выглядит как несоблюдённая пауза, но это её округление вверх до тика.

**Молчание API замораживает и состав пула, и перезапуски** (решено в `S1-09`). Пока `GET /internal/collector/assignments` не отвечает, менеджер не гасит процессы, не поднимает новые и не перезапускает упавшие. Гасить нельзя, потому что одна неудачная минута сняла бы все терминалы; поднимать — потому что процесс счёта берёт задание с паролем оттуда же и без связи умирает сразу, то есть пять попыток сгорели бы за пять минут и счёт остался бы лежать до перезапуска коллектора руками. Счёт с мёртвым процессом читает в это время «TradeDesk не отвечает — коллектор ждёт связи», а не обещание немедленного перезапуска.

**Крах самого менеджера уходит на карточки его счетов** (решено в `S1-09`). Перед выходом менеджер шлёт за счета, которым держал место, `state=error` с текстом про аварийную остановку (счетам сверх лимита — не шлёт: там уже стоит своя причина, и она осталась верной). Это единственный случай, когда о счёте рассказать буквально некому: `print` под Task Scheduler уходит в никуда, а без этого heartbeat на карточке осталось бы прежнее состояние, а через пять минут — «коллектор не на связи», то есть симптом вместо причины. Штатная остановка по-прежнему шлёт `state=stopped` и статуса не меняет — но штатная здесь значит «менеджер вышел сам»: Ctrl+C, `SIGTERM` или файл-просьба от `stop-collector.bat`. `TerminateProcess` — «Снять задачу», `taskkill /F`, принудительная ветка той же остановки — не шлёт ничего.

**Пауза и архив счёта гасят процесс жёстко, и недоотправленный батч теряется** (решено в `S1-09`). Потери данных нет: `last_sync_at` на сервере двигает только принятый батч, окно следующего запуска начинается от него минус сутки, вставка идемпотентна, а из нескольких частей окна уже принятые остаются принятыми. Цена другая и она названа в `docs/mt5-assumptions.md`, допущение 36: `mt5.shutdown()` при этом не вызывается, и портабельный `terminal64.exe` остаётся жить.
...
- **Жёсткое завершение менеджера оставляет процессы счетов сиротами.** `taskkill /F` и «Снять задачу» — обычный способ остановки под Task Scheduler — это `TerminateProcess`: ни обработчика сигнала, ни `atexit`, а `daemon=True` у дочерних процессов работает только через `atexit`, то есть при штатном выходе. Осиротевший процесс продолжает синхронизировать свой счёт, и следующий старт коллектора поднимет для него второй. Разбор и симптомы — `docs/mt5-assumptions.md`, допущение 39; защита (job-объект, зачистка при старте) — отдельная задача.
...
- Заведение счёта из списка (`S2-11`): та же форма, что на `/accounts`, в модальном окне. **Успех не закрывает окно молча**: человек завёл счёт, стоя на дашборде или в журнале, и под окном нет ничего, что бы ему ответило. Окно называет, что произошло (пароль зашифрован, коллектор возьмёт счёт в работу, сделок до первого синка нет), говорит, вошёл ли новый счёт в текущий выбор, и предлагает открыть его карточку или переключить выбор на него. **Выбор счетов сам не меняется** — это настройка рабочего места.
...
- **T‑01 SETUP.md финальный** — для не‑разработчика на Windows: Docker Desktop + WSL2, скачивание zip со страницы релизов (**не клонирование** — дистрибуция архивом, `v1.7` и ADR‑0005; Git на машине не нужен), `start.bat`, вход, установка Python и коллектора, `collector.env`, `install-service.ps1`, проверка статуса; скриншоты; раздел «Если что‑то не так» (коллектор не на связи, неверный пароль, счёт не в USD, Docker не стартует, порт занят). DoD: прогон на чистой Windows‑VM по инструкции без подсказок. ⚠️ **Скриншотов в `T‑01` нет, они отложены до `T‑06`**: снимать их надо на той машине, для которой написан текст, — окна `.bat`, Планировщик заданий, установщик Python, — а Windows у нас нет, и снимок с macOS, выданный за Windows, инструкцию не улучшает. Решение принципала по итогам ревью `T‑01`; полный список того, что в `SETUP.md` не проверено, — в самом документе, раздел «Что здесь проверено, а что нет».
