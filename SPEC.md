...
- Общий `refetchOnWindowFocus` остаётся выключенным, но **упавший** запрос перезапрашивается при возврате во вкладку: иначе ошибка залипает до F5 даже после того, как `api` доехал, — а человек, дождавшийся Docker Desktop, возвращается во вкладку именно за этим.
...
- «Приложение не поднято» отличается от «API сломался внутри» по ответу: запрос не доехал вовсе либо на 5xx нет тела §5.1 (так отвечает прокси, пока контейнер `api` стартует). `internal_error` с телом — живой сервер, и звать там проверять Docker Desktop было бы неправдой.
- У чтений есть общий предел ожидания (30 с): соединение, которое приняли и не ответили, иначе не падает вовсе. **У записи общего предела нет** — оборванная мутация неотличима от применённой; где предел записи нужен, его ставит вызывающий со своим текстом (карточка позиции, 15 с).
