...
- **Планировщик молчалив.** Задача, упавшая с ошибкой, выглядит в его списке так же, как отработавшая: об отказе человек узнаёт из карточки счёта («коллектор не на связи», до пяти минут) или из `logs/run-collector.log`, куда `run-collector.bat --service` перекладывает весь свой вывод. `install-service.ps1 -Status` печатает и то, и другое разом.
