...
**Сделка, которую граница отвергнет, не отправляется** (решено в `S1-08`). Коллектор проверяет то же, что и `S1-01`, — пробел в символе, расхождение `time`/`time_msc`, `entry` вне 0..3 — и такую сделку в батч не кладёт. Причина: батч отвергается целиком, и одна испорченная сделка остановила бы синхронизацию счёта **навсегда**, потому что из истории она никуда не денется. Потеря не молчаливая: тикет и причина уходят в лог и в heartbeat.
