...
Версия 1.5 — 4 сентября 2026 (v1.1: guard тестовой БД в DoD S0‑02, оговорка про инструментальные зависимости в 2.3; v1.2: домен `mail` в 2.1; v1.3: формат блобов credentials и `key_version` как отпечаток ключа, ADR‑0003; v1.4: инфраструктурные переменные окружения в 11.2; v1.5: словарь кодов ошибок в 5.1 приведён к реальности, ADR‑0004). Дополняет `PLAN.md`. Написана для разработки с AI‑агентами: каждый раздел самодостаточен, решения зафиксированы, неопределённости вынесены в раздел 14.
...
- Все ответы JSON. Ошибка:
  ...
  ```
  Коды HTTP: 400 валидация (`validation_error`, `details.fields`), 401 `unauthorized`, 403 `forbidden` и `forbidden_origin` (чужой `Origin` на мутирующем запросе, раздел 4), 404 `not_found`, 409 `conflict`, 422 бизнес‑правило (`unprocessable_entity` и доменные коды), 429 `rate_limited`.

  Порождаются фреймворком и окружением, а не доменом, но фронт обязан их знать: 405 `method_not_allowed`, 415 `unsupported_media_type`, 500 `internal_error` (наружу уходит только код и общее сообщение — текст исключения не выходит никогда).
...
`POST /ingest/deals` — заголовок `Authorization: Bearer <token>`, где token — либо сервисный токен коллектора (`COLLECTOR_TOKEN`, даёт доступ ко всем счетам, назначенным этому коллектору), либо персональный ключ счёта (`account_ingest_keys`, этап 4 для советника).
  ...
  "deals": [
    {
      ...
      "time_server": "2026-09-02T14:03:11", "time_msc": 1756821791123,
      "comment": "", "magic": 0
    }
  ],
...
6. Ставится задача worker'у `refresh_daily_stats(account_id, days_touched)`.
...
| GET | `/journal/positions/{id}` | Полная карточка: позиция, deals позиции, journal_entry, reflection, attachments (с presigned GET URL, TTL 1 ч) |
| PUT | `/journal/positions/{id}/entry` | Заметки, теги, planned_*, risk_amount |
| PUT | `/journal/positions/{id}/reflection` | Все поля рефлексии; `filled_at` ставится при первом сохранении с хотя бы одним заполненным полем |
...
| GET | `/journal/tags` / POST / DELETE | Словарь тегов |
| GET | `/journal/vocab` | Словари раздела 3.5 |
| GET | `/journal/calendar?month=2026-09&account_ids=` | `{days: [{day, trades, net_pnl, wins, losses, by_account: [{account_id, net_pnl, trades}]}]}` |

Правило: `result` считается по `net_pnl`: `> 0` win, `< 0` loss, `= 0` breakeven.

### 5.5 analytics (этап 3, в v1 — только календарь и summary)

`GET /analytics/summary?account_ids=&from=&to=` → `{trades, wins, losses, winrate, net_pnl, gross_pnl, commission, swap, profit_factor, avg_win, avg_loss, expectancy, best_trade, worst_trade, open_positions}`. Формулы — `docs/metrics.md`. Используется в шапке журнала.
...
- `GET /users/me` — то же, что `auth/me`; `PATCH /users/me {display_name?, timezone?, day_boundary_hour?}` — таймзона валидируется по IANA, час 0–23.
...
3. Логи — structlog в файл `logs/collector.log` с ротацией; в логах никогда нет пароля.
...
**Accounts.** Карточки счетов: цветная полоса, метка, демо‑бейдж, брокер/сервер/логин, статус (`connected` зелёный с «синк 1 мин назад», `pending` серый «ожидает коллектор», `needs_attention` красный с `status_message`, `paused`), кнопки «Синхронизировать», «Пауза», меню «Изменить / Архивировать / Удалить». Форма добавления — модал: платформа (MT5 / Вручную), метка, демо, цвет (палитра), для MT5 — сервер, логин, investor‑пароль с подсказкой «только пароль инвестора; он не позволяет торговать». Блок «Коллектор»: `last_heartbeat_at`, инструкция‑ссылка на `SETUP.md`, если heartbeat старше 5 минут — жёлтое предупреждение «коллектор не на связи».

**Journal.** Панель фильтров (период с пресетами: сегодня, неделя, месяц, всё; символ; направление; результат; теги; «без рефлексии»; поиск). Таблица (виртуализированная): цвет счёта | дата закрытия | символ | направление | объём | вход → выход | длительность | net P&L (цвет) | R (если есть) | теги | иконки: рефлексия заполнена / скриншот. Клик — карточка. Сверху — summary за фильтр (`/analytics/summary`). Фильтры — в URL query. Мобильный (< 768px): список карточек с теми же данными в 2 строки.
...
|---|---|---|
| `refresh_daily_stats(account_id, days)` | после ингеста, после ручной сделки/правки | Пересчёт затронутых дней в `daily_stats` (таблица, не MV — обновление точечное) |
...
- `stop.bat|sh`.
- `update.bat|sh` — `backup` → `git pull` → `docker compose build` → `up -d` (миграции применяет `api` при старте) → вывод версии.
...
- **S1‑09 Коллектор: менеджер процессов, heartbeat, assignments** — `main.py`, лимит, остановка при паузе. DoD: два счёта одновременно; пауза одного останавливает его процесс.
- **S1‑10 Коллектор: установка** — `run-collector.bat`, `install-service.ps1`, читаемые ошибки MT5. DoD: после перезагрузки Windows коллектор поднялся сам; неверный пароль виден в UI как «Неверный пароль инвестора».
...
- **S2‑09 UI Calendar** — 9.3. DoD: суммы дней совпадают с журналом за тот же день; разбивка по счетам при наведении.
- **S2‑10 UI Dashboard v1** — 9.3 Dashboard, пустое состояние с онбордингом. DoD: новый пользователь видит шаги; после подключения счёта галочки проставляются.
