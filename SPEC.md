...
**Упавший процесс перезапускается пять раз подряд, дальше — отказ словами** (решено в `S1-09`). Пауза между попытками удваивается от 5 с до 15 мин; процесс, проживший 10 минут, обнуляет счёт попыток. Коды выхода 2 (`collector.env`) и 3 (не Windows) не перезапускаются вовсе — у нового процесса будут те же настройки и та же ОС. Исчерпанные перезапуски и отказ по конфигурации видны на карточке счёта разными текстами: в первом случае человеку нужен файл лога, во втором — правка `collector.env`.
