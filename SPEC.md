...
- Терминал должен успеть загрузить историю с сервера брокера: после `initialize` коллектор ждёт до 30 с, пока `history_deals_total` не перестанет расти между двумя вызовами. **Ожидание необязательное, и его отказ не останавливает ничего** (`X-74`): не дождались — в файл лога уходит предупреждение `collector.history_wait_failed`, синхронизация идёт дальше, первый батч может уехать неполным, а следующий тик его дозаполнит (вставка идемпотентна). Обратное измерено на живой Windows 16 сентября 2026: границы счётчика уезжают в нативную библиотеку, та переводит наивный `datetime` в системное время Windows, и у самой эпохи (`1970-01-02`) перевод отвечает `EINVAL` — наружу шёл `SystemError`, мимо всех наших классов ошибок, и косметический шаг останавливал коллектор целиком вместе со **всеми** счетами. Границы поэтому названы числом: от `2000-01-01` (старше и MetaTrader 5, и потолка `FIRST_SYNC_DAYS` в 7300 дней) до «сейчас + 2 суток» (сутки вперёд, как у окна выборки, плюс максимум реальной зоны из §6.3 — границы сравниваются с часами брокера, а не с UTC).
