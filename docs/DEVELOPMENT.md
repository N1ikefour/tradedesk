...
**Как починить — не таймингом, а ассертом.** Прогрев сессий (`select 1` под тем же `gather`) гонку восстанавливает, но держит её **тайминг**, а тайминг ничем не закреплён: ленивое получение сессии или переезд теста на HTTP-уровень вернут ложную зелень молча.

Правильный способ — **утверждать сам факт одновременности**. Третья сессия-ворота берёт `SELECT … FOR UPDATE` на те же строки, обе проверяемые сессии упираются в замок, тест опрашивает `pg_locks`, пока ждущими не окажутся **обе**, ассертит это — и только потом отпускает ворота.

Так исход «гонка была, блокировка сработала» отличим от «гонки не было вовсе», а без ворот окно ожидания длится единицы миллисекунд, и опрос делал бы тест флаки по построению.

⚠️ Читать надо `pg_locks`, а не `pg_stat_activity`: последний внутри открытой транзакции отдаёт снимок, сделанный один раз, и опрос в цикле будет возвращать одно и то же до `pg_stat_clear_snapshot()`.

⚠️ `asyncio.Barrier` эту задачу не решает: он синхронизирует вход в питоновский вызов, а не встречу двух `UPDATE` внутри Postgres, и его режим отказа — зависание до таймаута, а не красный ассерт.
