...
| X-69 | Цитаты `SETUP.md` не сверяются с текстами кода ничем | infra | **Done** | [PR #43](https://github.com/N1ikefour/tradedesk/pull/43) · остаток сличителя в `X-72` |
...
| X-73 | 🔴 Проверка версии после обновления не работает на Windows никогда | infra | Todo | — · воспроизведено 16.09 у пользователя |
| X-75 | Нативный вызов MT5 роняет процесс мимо `TerminalError` | collector | Todo | — · найдено ревью `X-74` |
| X-74 | 🔴 Коллектор падает на Windows в `wait_for_history`: дата 1970 года | collector | **Done** | [PR #46](https://github.com/N1ikefour/tradedesk/pull/46) · класс шире — в `X-75` |
...
| Коллектор | ~~`S1-08`~~, ~~`S1-10`~~, ~~`T-07`~~, ~~`X-66`~~ | **переписан по итогам живого прогона**: терминал открывает человек, коллектор подключается к открытому, пароля в продукте нет (ADR-0006). 10 сентября на машине пользователя дошли до запуска, но **ни одной сделки не синхронизировали** — это и есть предмет `T-06`. `S1-09` (менеджер процессов) отменён вместе с пулом |
| Доставка | ~~`X-19`~~ → ~~`T-02`~~ → ~~`T-01`~~ → ~~`T-08`~~ | **закрыт и проверен на человеке**: `v0.1.0` опубликован 9 сентября, скачан анонимно, сумма сошлась, по нему и ставили. Инструкция переписана по итогам прогона (`T-08`), и теперь её цитаты сверяет с кодом тест (`X-69`) |
...
| Тексты на экране | ~~`X-62`~~, ~~`X-70`~~, ~~`X-42`~~ | **закрыт**: блок «Коллектор» приведён к тому, что делает код, и это стережёт тест. `X-70` вычистил хвосты отменённой архитектуры из того, что читает человек, — включая **две неправды, печатавшиеся на пути `backup` и `update`**, то есть в момент решения о данных. `X-42` убрал вечную «Загрузка…»: открыв страницу раньше, чем Docker поднял контейнеры, человек теперь видит объяснение и кнопку, а не зависшее приложение. Заодно вычищено обещание «на карточках появится „Коллектор остановлен"» — его печатали скрипт остановки, `SETUP.md`, чек-лист и `SPEC.md`, а сервер такого не пишет никогда |
...
| X-13 | Приостановленный запрос выглядит как вечная загрузка | frontend | **Done** | [PR #45](https://github.com/N1ikefour/tradedesk/pull/45) · вместе с `X-42` |
...
| X-42 | Экран без сети показывает вечную «Загрузка…» (+ `X-13`) | frontend | **Done** | [PR #45](https://github.com/N1ikefour/tradedesk/pull/45) |
