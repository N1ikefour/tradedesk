...
| Windows-пути | `X-43`, `X-54` | у друга имя пользователя кириллицей. ⚠️ `X-54` **оказался мягче, чем думали**: решает имя самой папки установки, а зип распаковывается в `tradedesk-<версия>` латиницей — на macOS такой путь собирается. Наша проверка всё равно отказывает (на Windows не прогонялось), то есть у друга будет лишний шаг «перенесите папку» |
