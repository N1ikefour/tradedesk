...
| Доставка | ~~`X-19`~~ → ~~`T-02`~~ → `T-01` | зип и скрипты готовы; осталась инструкция. ⚠️ **Релизов опубликовано ноль** — тег ставит принципал, когда всё будет доделано |
| Установка | ~~`X-17`~~, `X-61` | `init.bat` есть, Git Bash больше не нужен. ⚠️ Но `start.bat` двойным щелчком закрывает окно вместе с адресами приложения и текстом ошибки — у главного шага нет видимого результата. Мелкая правка, делаем до передачи |
...
| Тексты на экране | `X-62` | ⚠️ блок «Коллектор» на экране «Счета» утверждает, что коллектора ещё нет, — а `SETUP.md` шлёт человека на этот экран ставить коллектор. Инструкция права, приложение врёт. **До тега** |
| Журнал | ~~`S2-09`~~, `S2-11` | календарь работает, заглушек в приложении не осталось; счёт по-прежнему заводится только на экране счетов |
...
| X-61 | `start.bat` закрывает окно вместе с адресами и ошибкой | infra | Todo | — |
| X-62 | Экран «Счета» говорит про коллектор неправду | frontend | Todo | — |
