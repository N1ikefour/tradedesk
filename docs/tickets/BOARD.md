...
| Коллектор | ~~`S1-08`~~, ~~`S1-09`~~, `S1-10` | процесс счёта и менеджер написаны; осталась **установка**. Проверить — только на Windows с терминалом |
...
| Установка | ~~`X-17`~~ | закрыт: `init.bat` есть, Git Bash больше не нужен |
...
| Журнал | `S2-09`, `S2-11` | календарь ещё заглушка; счёт заводится только на экране счетов. `S2-03`, `S2-04`, `S2-08` — по решению принципала после первого прогона |
