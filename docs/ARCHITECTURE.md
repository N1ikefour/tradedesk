...
| Задача | Что даёт | Состояние |
|---|---|---|
| `S0-01` | структура монорепо, линтеры, `make ci`, GitHub Actions | смержено |
| `S0-02` | `app/core` (config, db, redis, logging, errors), `domains/system` с `/health` и `/version`, Alembic без ревизий, guard тестовой БД, env-guard прода | смержено |
| `S0-03` | модель данных: 13 таблиц одной ревизией, UUID v7, ER-диаграмма ниже | в ревью |

Ещё не существует: `auth`-флоу (S0-04), шифрование credentials (S0-05), Docker Compose (S0-06), фронтенд (S0-07), коллектор (этап 1), прод.

Разделы про repo map, слои API и обработку ошибок пока не написаны — их место в «Порядке заполнения» ниже, и они не заполнены задним числом намеренно: `DEVELOPER_MANUAL.md` §10.2 требует писать сюда проверенное, а не восстановленное по памяти.
