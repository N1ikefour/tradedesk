...
# пароли счетов человек может ввести заново. Но узнать он должен ДО подтверждения.
...
if [ -z "$KEYS_IN_DUMP" ]; then
  ...
  case "$(printf '%s' "$VERDICT" | cut -f1)" in
    ...
    *)
      ...
      td_say "              Журнал, сделки и рефлексии восстановятся полностью — а пароли"
      td_say "              счетов придётся ввести заново в настройках. Восстановление"
      td_say "              этого не чинит: ключ лежит в .env, а не в базе."
      ;;
  esac
fi
td_say ""
