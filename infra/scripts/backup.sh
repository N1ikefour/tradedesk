...
#
# ⚠️ Дамп без .env неполон. Журнал, сделки и рефлексии он вернёт и так, а вот то, что
# зашифровано MASTER_KEY, из него уже не прочитать: ключ живёт только в .env и в дамп не
# попадает — про это скрипт говорит вслух каждый раз.
set -eu
...
note ""
note "⚠️  Дамп без .env неполон: зашифрованное MASTER_KEY из него не прочитать, а ключ"
note "    лежит только в .env и в дамп не попадает. Храните копию .env рядом с бэкапами."
