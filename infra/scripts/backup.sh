...
#
# Удалять есть что только когда файлов больше $KEEP, и только среди наших имён. Файл,
# созданный этим прогоном, проверен выше и стоит первым в списке — значит, ротация
# физически не может оставить установку без единого целого бэкапа.
if [ "$KEEP" -gt 0 ]; then
  ALL="$(ls -t "$BACKUP_DIR"/td-*.sql.gz 2>/dev/null || true)"
  TOTAL="$(printf '%s' "$ALL" | grep -c . || true)"
  if [ "${TOTAL:-0}" -gt "$KEEP" ]; then
    OLD="$(printf '%s\n' "$ALL" | tail -n +"$((KEEP + 1))")"
    printf '%s\n' "$OLD" | while IFS= read -r stale; do
      [ -n "$stale" ] || continue
      [ "$stale" != "$FINAL" ] || continue
      rm -f "$stale"
      note "  удалён старый бэкап: $(basename "$stale")"
    done
    note ""
    note "Оставлено последних бэкапов: $KEEP."
  fi
fi
