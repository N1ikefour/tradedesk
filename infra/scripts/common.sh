...
#
# ⚠️ Признак ровно один, и запасного к нему нет намеренно. Запасным напрашивается
# OS=Windows_NT — переменная, которую cmd.exe выставляет всем и которую Git Bash
# наследует, — но она страхует от того, чего не бывает. Она бы вступала в дело, только
# если бы `uname` не нашёлся; `uname` лежит в Git\usr\bin рядом с grep, sed, cut и tail,
# без которых ни один скрипт установки не доходит до второго десятка строк. Машины, где
# пропал ровно `uname`, а соседи по той же папке на месте, не существует.
#
# Зато цена признака вполне реальна: `OS` — имя без префикса TD_, чужое и общее, а от
# него зависят ВСЕ подсказки, которые человек читает в момент действия (td_cmd). Один
# случайно унаследованный `OS=Windows_NT` — и установка на macOS советует .bat-файлы.
# Признак, который ничего не страхует, но умеет соврать, лучше не иметь.
td_is_windows() {
...
в .env лежит MASTER_KEY — ключ шифрования установки, и второго экземпляра нет.
...
# ⚠️ ADR-0005. Без .env нет MASTER_KEY, а без него всё, что им зашифровано в томе,
# нечитаемо навсегда. Молча создать новый .env здесь — худшее, что может сделать скрипт.
td_require_env() {
  if [ ! -f "$TD_ROOT/.env" ]; then
    td_die "Нет $TD_ROOT/.env — обновлять и резервировать нечего.

Это не «сейчас создадим»: в .env лежит MASTER_KEY — ключ шифрования этой
установки, и второго экземпляра у него нет. Новый ключ откроет приложение, но не
откроет ничего из зашифрованного старым: сохранённые пароли счетов останутся
нечитаемыми навсегда.
...
Если .env потерян — восстановите его из своей копии. Другого пути нет."
  ...
  td_env_present MASTER_KEY ||
    td_die "В $TD_ROOT/.env пуст MASTER_KEY — ключ шифрования этой установки.
