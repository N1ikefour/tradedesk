...
# старой, приложение поднимается, журнал на месте — а всё, что зашифровано старым ключом,
# становится нечитаемым молча и навсегда. Заметно это не в момент ошибки, а сильно позже,
# когда вернуть уже нечем. Сверять есть с чем:
...
usage() {
  td_warn "Использование: $(td_script update) [--check] [--version vX.Y.Z] [--file <zip>] [--yes]"
  td_warn ""
  td_warn "  без аргументов   обновиться до последнего релиза"
  td_warn "  --check          только проверить: версия, MASTER_KEY. Ничего не менять"
  td_warn "  --version vX.Y.Z поставить конкретную версию"
  td_warn "  --file <zip>     взять архив с диска вместо скачивания (нет интернета —"
  td_warn "                   скачайте zip вручную со страницы релизов и укажите его)"
  td_warn "  --yes            не спрашивать подтверждения"
}
...
# из релиза, а рабочая копия репозитория, — и обновлять её распаковкой архива поверх
# исходников нельзя.
#
# ⚠️ Никакой команды здесь не называется намеренно (X-64). Отказ читают двое, и рецепт у
# них разный: разработчику нужны средства разработки, а человеку, у которого установка
# оказалась клоном, — распаковать релизный архив (ADR-0005). Одна строка, верная для
# обоих, не пишется, а неверная тем дороже, чем ближе она к папке с данными.
...
Рабочую копию этот скрипт не обновляет — обновляйте её обычными средствами
разработки (README.md). Установка обновляется только из релизного архива:
её распаковывают со страницы релизов, а не клонируют (SETUP.md, шаг 2)."
fi
CURRENT="$(tr -d ' \t\r\n' <"$TD_ROOT/VERSION")"
...
if [ -f "$MARKER" ]; then
  ...
  if [ "$CHECK_ONLY" = "1" ]; then
    td_say "  --check: ничего не менялось. Обновление не доделано — запустите"
    td_say "  $(td_cmd update) без --check, чтобы довести его до конца."
    exit 0
  fi
...
без этой сверки оно может сделать зашифрованное этим ключом нечитаемым (ADR-0005)."
...
её там: в новой папке нет .env, $(td_cmd init) создал новый MASTER_KEY, а база
осталась прежней. Приложение бы поднялось и журнал был бы на месте — а
сохранённые пароли счетов стали бы нечитаемы молча и навсегда.

Что делать:
  1. Найдите папку СТАРОЙ установки — ту, из которой приложение работало.
  2. Запустите обновление оттуда: там лежит правильный .env.
  3. Если старый .env потерян — эти пароли не вернуть ничем, включая бэкап
     базы. На синхронизацию это не влияет: в MetaTrader 5 вы входите сами.
...
Установка не тронута."
  ...
  # объяснять, куда делся MASTER_KEY: перенос затёр бы его вместе со всем зашифрованным.
...
запустите $(td_cmd update) ещё раз, он продолжит перенос."
    MOVED=$((MOVED + 1))
  done 3<"$FILES"
...
Не запускайте $(td_cmd init): он создаст НОВЫЙ MASTER_KEY, и зашифрованное старым
станет нечитаемым навсегда. Верните .env из своей копии."
...
база не тронута и бэкап лежит здесь:
    $BACKUP_FILE
Исправьте причину и запустите $(td_cmd update) ещё раз — он продолжит со сборки."
...
Починив причину, запустите $(td_cmd update) ещё раз — он продолжит со сборки."
...
Что делать:
  ...
  2. запустите $(td_cmd update) ещё раз — он продолжит со сборки и запуска.
