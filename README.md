...
| `apps/collector-mt5` | Процесс одного счёта (`S1-08`), менеджер процессов над ним (`S1-09`) и установка (`S1-10`): `run-collector.bat`, автозапуск через Планировщик заданий (`install-service.bat`), остановка (`stop-collector.bat`). **Запускается только на Windows** — библиотеки `MetaTrader5` под macOS и Linux не существует, и на живом терминале ничего из этого ещё не выполнялось. Что и как проверить на Windows — `docs/collector-windows-checklist.md` |
