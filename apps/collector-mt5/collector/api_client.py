...
**Пароля счёта здесь нет вовсе** (`T-07`, `X-66`). В терминал коллектор не входит —
терминал открывает человек, — и API пароля не отдаёт: его нет ни в поле `Assignment`, ни в
ответе `/internal/collector/assignments`. Реестр скраба (`logging_setup`) остаётся, но
секрет в нём теперь один — `COLLECTOR_TOKEN`.
...
def _assignment(item: object) -> Assignment:
    ...
    try:
        ...
        # Текст исключения не подставляется: `KeyError` печатает ключ, `ValueError` от
        # `int()` — значение целиком, и оба по-английски. Это сообщение уезжает в файл лога
        # человека (`main._assignments`), а причин у отказа ровно две, и обе названы здесь
        # словами. Само исключение остаётся в цепочке `__cause__` для трейсбека.
        raise ApiError(
...
class HeartbeatAccount:
    ...
    def payload(self) -> dict[str, Any]:
        ...
        if self.message is not None:
            ...
            # чтобы секрета не было и в текстах ошибок, а `status_message` видно на экране.
