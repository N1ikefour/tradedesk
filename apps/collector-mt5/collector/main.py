...
# `TerminateProcess`: цикл до `_shutdown` не доходит, то есть процессы счетов остаются
# сиротами (`X-57`, допущение 39), а прощальный heartbeat не уходит вовсе. Файл —
# единственная просьба, которую менеджер может услышать сам.
