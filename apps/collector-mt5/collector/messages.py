...
# `account_info` батч не существует, и синк счёта стоит до починки.
...
# --------------------------------------------------------------------------------------
# Менеджер процессов (`main.py`, S1-09). Всё, что здесь, человек читает про счёт, у
# которого нет живого процесса **и который не успел объяснить свой уход сам**. Процесс,
# вышедший с кодом 4, причину уже назвал — там менеджер молчит (`pool.MemberStatus`).
# --------------------------------------------------------------------------------------

# SPEC.md 12 (S1-09) требует именно этого: превышение лимита обязано быть видно человеку,
# а не съедено молча. Число подставляется в скобках, а не в оборот «ведёт {limit} счетов»,
# ровно затем, чтобы фраза оставалась грамотной при любом значении.
OVER_LIMIT: Final = (
    "Счёт не синхронизируется: у коллектора заняты все места (MAX_ACCOUNTS={limit} в "
    "collector.env). Увеличьте лимит и перезапустите коллектор — терминал занимает 300–400 МБ."
)

WORKER_RESTARTING: Final = (
    "Процесс счёта неожиданно завершился (код {code}), коллектор запускает его заново. "
    "Если сообщение повторяется, пришлите разработчику файл лога счёта из папки LOG_DIR."
)

# Перезапуск заморожен: без списка счетов от TradeDesk менеджер не трогает состав пула
# (`main.Manager._apply`). Обещать в этот момент «запускаю заново» было бы неправдой —
# менеджер не поднимет процесс, пока не дозвонится.
WORKER_RESTART_DEFERRED: Final = (
    "Процесс счёта неожиданно завершился (код {code}), а TradeDesk не отвечает — коллектор "
    "ждёт связи и не перезапускает его. Проверьте, что TradeDesk запущен и доступен."
)

WORKER_NOT_STARTED: Final = (
...
WORKER_GAVE_UP: Final = (
    "Процесс счёта падает при каждом запуске (попыток: {attempts}, последний код {code}). "
    "Коллектор больше не поднимает его — нужен файл лога счёта из папки LOG_DIR."
)

WORKER_FATAL: Final = (
    "Процесс счёта не смог стартовать (код {code}): ошибка в collector.env или система не "
    "Windows. Перезапуск не поможет — смотрите collector.log в папке LOG_DIR."
)

MANAGER_CRASHED: Final = (
...
CRASHED: Final = (
    "Коллектор аварийно остановился ({error}). Подробности — в файле лога счёта в папке "
    "LOG_DIR; пришлите их разработчику."
)
...
def over_limit(limit: int) -> str:
    return fit(OVER_LIMIT.format(limit=limit))
