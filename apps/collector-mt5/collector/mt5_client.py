...
1. **Никаких решений.** Функции забирают данные и переводят коды ошибок в человеческий
   ...
   вопрос «наш ли это счёт» — последний живёт в `identity.py`. Четыре решения, которые
   здесь всё-таки принимаются, вынесены в чистые функции модуля и проверены тестами: какой
   тик считать свежайшим (`freshest_tick_time`), означает ли `None` от `positions_get()`
   пустоту или отказ (`positions_mean_empty`), догрузилась ли история (`history_step`) и в
   каких границах её считать (`history_count_window` — `X-74`: дата, уехавшая в библиотеку,
   на Windows останавливала коллектор, и вынести её под тест оказалось единственным
   способом хоть что-то о ней доказать).
...
# Нижняя граница счётчика сделок. Не «день после эпохи»: наивный `datetime` библиотека
# переводит в системное время средствами Windows, и у самой эпохи этот перевод возвращает
# `EINVAL` — на живой машине `1970-01-02` уронила коллектор целиком (`X-74`). 2000 год
# старше любой сделки, какая может лежать в терминале (MetaTrader 5 вышел в 2010), и
# старше самого широкого окна, какое коллектор вправе прочитать: `FIRST_SYNC_DAYS`
# ограничен 7300 днями (`config.py`), то есть двадцатью годами назад.
HISTORY_COUNT_FROM: Final = datetime(2000, 1, 1, tzinfo=UTC).replace(tzinfo=None)
# Верхняя граница — ровно тот запас, с которым историю читают на самом деле: сутки вперёд
# (`sync.LOOKAHEAD`) плюс максимум реальной зоны, 14 часов (`sync.MAX_OFFSET_MINUTES`),
# потому что границы сравниваются с часами брокера, а не с UTC. Прежний год вперёд запасом
# не был: сделок там не бывает, а дата, которую платформа в обычной жизни не встречает, —
# это ещё один перевод времени, проверить который негде.
HISTORY_COUNT_AHEAD: Final = timedelta(days=2)
...
def history_count_window(now: datetime) -> tuple[datetime, datetime]:
    """Границы, в которых считаются сделки при ожидании догрузки истории (`SPEC.md` §8.3).

    Решение вынесено сюда потому, что обе даты уходят в нативную библиотеку, а она
    переводит их в системное время Windows — тем самым переводом, который на `1970-01-02`
    вернул `EINVAL` и остановил коллектор (`X-74`). Сам перевод на машине разработки не
    воспроизводится вовсе; проверить можно только сами даты, и для этого их надо вынуть из
    непроверяемого метода.

    Границы обязаны накрывать всё, что синхронизация вправе прочитать: счётчик, не видящий
    приехавших сделок, объявит историю догрузившейся раньше времени. Обе — без зоны:
    `MetaTrader5` принимает наивный `datetime` и зону в нём молча игнорирует.
    """
    return HISTORY_COUNT_FROM, now.astimezone(UTC).replace(tzinfo=None) + HISTORY_COUNT_AHEAD
...
class Mt5Terminal:
    ...
    def wait_for_history(self) -> None:
        ...
        Шаг необязательный, и звать его положено так, чтобы отказ ничего не останавливал:
        сторож стоит у вызывающего (`main._settle_history`, `X-74`).
        """
        mt5 = self._module()
        start, end = history_count_window(datetime.now(UTC))
        previous = -1
        waited = 0.0
