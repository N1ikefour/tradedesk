...
# _shutdown, то есть счета не получают state=stopped и через пять минут уезжают в
# «коллектор не на связи» после осознанного действия человека.
...
function Stop-Collector {
    ...
    if ($left.Count -gt 0) {
        ...
        Write-Note 'На карточках счетов в TradeDesk должно быть «Коллектор остановлен».'
        Write-Note 'Если вместо этого через пять минут появится «коллектор не на связи» —'
        Write-Note 'прощание не доехало. Это не поломка и не потеря данных: следующий'
        Write-Note 'запуск заберёт своё окно целиком.'
    } else {
