...
function Install-Task {
    ...
    Write-Note '«Снять задачу» в диспетчере задач тоже останавливает — процесс у коллектора'
    Write-Note 'один, и сирот после неё не остаётся. Но выйти по-человечески он при этом'
    Write-Note 'не успевает: канал к терминалу не отпускается. stop-collector.bat лучше.'
