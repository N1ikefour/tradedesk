  ...
  return (
    <header className="border-b border-border bg-card">
      <div className="mx-auto flex h-14 max-w-7xl items-center gap-4 px-4">
        ...
        <nav aria-label={t.header.menu} className="flex items-center gap-1 overflow-x-auto">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
        ...
        <div className="ml-auto flex items-center gap-2">
          {/*
            ...
            не перекраивалась под него задним числом.
          */}
          <div
            data-slot="account-switcher"
            aria-hidden="true"
            title={t.header.accountSwitcherHint}
            className="hidden h-9 items-center rounded-md border border-dashed border-border px-3 text-sm text-muted-foreground sm:flex"
          >
            {t.header.accountSwitcherPlaceholder}
          </div>
