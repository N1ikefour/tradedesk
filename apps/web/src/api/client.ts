 ...
 *
 * ⚠️ Предел жёсткий: вызывающий может попросить меньше своим сигналом, но не больше, и
 * механизма исключения нет. Первым в потолок упрётся «Экспорт всех данных» (SPEC.md 9.3,
 * Settings) — он собирает JSON-архив по всем данным пользователя, и тридцати секунд ему
 * может не хватить. Его автору: исключение заводить здесь, а не обходить клиент стороной.
 */
