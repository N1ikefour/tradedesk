export interface paths {
  ...
  '/api/v1/internal/collector/assignments': {
    ...
    /**
     * Assignments
     * @description За какими счетами следить. **Пароля в ответе нет** (`T-07`, ADR-0006).
     *
     ...
     *     два коллектора в одной сети получили бы одни и те же счета и оба полезли бы в один
     *     брокерский аккаунт.
     */
...
export interface components {
  schemas: {
    ...
    AccountUpdateRequest: {
       ...
       * Password
       * Format: password
       * @description Устарело (T-07): коллектору пароль не нужен — в терминал входит человек. Присланное значение шифруется и хранится, но не читается ничем и ни в одном ответе не возвращается
       */
      password?: string;
    };
    /**
    ...
    /**
     * AssignmentResponse
     * @description Задание коллектору на один счёт — SPEC.md 5.6. **Пароля не содержит.**
     */
    AssignmentResponse: {
      /**
       * Account Id
      ...
      /**
       * Login
       * @description Номер счёта. Коллектор сверяет с ним счёт, открытый в терминале: подключение идёт к терминалу человека, и чужой счёт иначе попал бы в этот журнал молча
       */
      login: number;
      /**
       * Sync Requested At
