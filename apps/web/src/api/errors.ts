...
export class NetworkError extends Error {
  ...
  /**
   * Запрос оборвали мы сами — по своему пределу ожидания, а не сетью. Признак нужен ровно
   * одному читателю, `messageForError`, и ровно из-за записи: оборванную мутацию сервер
   * мог уже применить, и «проверьте соединение» отправляло бы чинить то, что не сломано.
   * На чтении разницы нет — см. `isServerUnreachable`.
   */
