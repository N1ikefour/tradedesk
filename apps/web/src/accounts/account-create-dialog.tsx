 ...
 *
 * Форма — та же самая, что на `/accounts`: набор полей и их проверки существуют в одном
 * экземпляре, копия разошлась бы с оригиналом на первой же правке.
 *
 ...
 * и там под окном нет ничего, что бы ему ответило. А произошло многое: счёт попал в задания
 * коллектору, но сделок у него не будет до первого синка, а первый синк не начнётся, пока
 * человек не откроет этот счёт в терминале (`T-07`), — и в текущий выбор счетов он мог не
 * войти вовсе. Поэтому окно не закрывается молча, а называет всё это и предлагает два
 * продолжения: посмотреть, как счёт подключается, или переключить на него выбор.
 *
...
export function AccountCreateDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal
      ...
      className="max-w-xl"
    >
      {/* Форма пересоздаётся при каждом открытии: прошлый ввод не должен пережить закрытие
          окна. */}
