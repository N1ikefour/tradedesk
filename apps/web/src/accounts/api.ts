 ...
 * есть достижимы и обе копии. Поэтому обе стираются сразу после успеха, а не по таймеру.
 *
 * ⚠️ Заведено это было под открытый пароль счёта в теле, а его в теле больше нет (`T-07`).
 * Механизм оставлен: на нём же держится признак «сохранено» в форме, и снятие его — отдельная
 * правка, а не побочный эффект этой.
 */
