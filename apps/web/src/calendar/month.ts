 ...
 * её отсутствия на экране: месяц за пределами не подрезается до ближайшего допустимого, а
 * игнорируется целиком — `2101-01` даёт не `2100-12`, а текущий месяц. Подрезка соврала бы
 * про то, что человек открыл; так на сервер уходит заведомо принимаемый запрос, а человек
 * видит свой календарь вместо ошибки.
 */
import { t } from '@/i18n';
 ...
 * не разбор её синтаксиса (то же правило, что у фильтров журнала). В адресе мусор при этом
 * не остаётся — его убирает экран (`routes/calendar-page.tsx`), иначе убрать его нечем.
 */
