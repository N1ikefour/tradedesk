...
def test_dotted_capital_i_is_treated_as_the_i_suffix() -> None:
    """Единственная точка, где поведение расходится с буквальным чтением SPEC.md 6.4.

    Свип всех кодовых точек против «поднять регистр и у альтернатив тоже» даёт ровно одно
    расхождение: `İ` (U+0130) IGNORECASE считает вариантом `I` и срезает как суффикс, а
    буквальное чтение оставило бы имя целиком. Расхождение зафиксировано, а не починено:
    у брокеров такого тикера нет, а точное совпадение стоило бы читаемости шаблона.
    Тест здесь затем, чтобы смена этого решения была видна, а не случилась молча.
    """
    assert clean_symbol("EURUSD.İ") == "EURUSD"
