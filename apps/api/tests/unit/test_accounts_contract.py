...
def test_password_is_accepted_only_on_the_way_in(document: dict[str, Any]) -> None:
    """Обратная сторона: поле осталось входом, и остаётся `writeOnly`.

    С `T-07` счёт заводится без пароля, но поле из тела запроса не убрано — SPEC.md 11.4
    требует вперёд-совместимых правок, и первый шаг это «перестать требовать». Проверка
    держит вторую половину: пока поле объявлено, оно обязано быть только на вход, иначе
    генератор клиента положит его в тип ответа.
    """
