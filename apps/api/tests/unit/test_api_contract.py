...
async def error_probe_client() -> AsyncIterator[AsyncClient]:
    ...
    Здесь проверяются свойства самих обработчиков — на маршруте, который можно сломать
    как угодно. Соответствие боевых маршрутов объявленному в OpenAPI сверяет
    `test_openapi_errors.py` (X-03).
    """
    app = FastAPI()
