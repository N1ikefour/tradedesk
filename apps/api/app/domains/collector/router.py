...
GET  /api/v1/internal/collector/assignments  -> 200 {items}

Авторизация объявлена на роутере, а не на маршрутах: маршрут, забывший зависимость,
открыл бы наружу состав чужой установки — счета, серверы брокеров и номера счетов, — и
полагаться тут на внимательность нельзя. Любой будущий маршрут этого файла наследует
проверку сам.
...
async def assignments(
    ...
    """За какими счетами следить. **Пароля в ответе нет** (`T-07`, ADR-0006).
    ...
    return AssignmentListResponse(items=[AssignmentResponse.issued(account) for account in issued])
