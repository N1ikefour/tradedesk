...
2. **Пароль живёт в `account_credentials` и наружу не выходит вовсе.** Здесь он только
   пишется, и с `T-07` (ADR-0006) его не читает никто: в терминал MT5 входит человек, а
   коллектор подключается к уже открытому. Поле осталось необязательным входом ради
   вперёд-совместимости (SPEC.md 11.4) и ради строк, заведённых до этого решения.
...
# Поля, смена которых означает другой счёт у брокера: прежнее «подключён» к нему не
# относится, и подтвердить его заново может только следующий синк.
...
async def create_account(
    ...
    """Создаёт счёт со статусом `pending`. Пароль необязателен и обычно не приходит.
...
def _resets_connection(account: models.TradingAccount, changes: dict[str, Any]) -> bool:
    """Требует ли правка заново подтвердить, что счёт синкается.

    Только **изменение** `server` или `login`: это другой счёт у брокера, и прежнее
    «подключён» к нему не относится. Разница между «изменилось» и «присутствует в теле»
    принципиальная — форма редактирования шлёт карточку целиком, и второе означало бы,
    что переименование счёта каждый раз роняет работающий синк в `pending`.

    Пароля в этом списке нет с `T-07`: коллектор в терминал не входит, и менять из-за
    пароля состояние счёта значило бы обещать перезаход, которого не будет.
    """
    return any(
...
async def update_account(
    ...
    if resets:
        # Пауза переживает смену сервера и логина: возобновление синка — отдельное решение
        ...
        # Причина прошлого `needs_attention` относилась к прежней паре сервер+логин.
