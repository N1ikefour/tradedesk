...
class AccountCredential(Base):
    """Шифротекст, у которого с `T-07` не осталось ни одного читателя (ADR-0006).

    В терминал MT5 входит человек, коллектор подключается к открытому, и пароль счёта не
    отдаёт больше ни один эндпоинт — `assignments` в том числе. Строки, сохранённые до
    этого решения, лежат здесь зашифрованными; таблица уходит отдельной задачей, чтобы
    правка оставалась вперёд-совместимой (`SPEC.md` §11.4). `decrypt_credentials` без
    вызовов — это состояние, а не пробел: новый вызов означает возврат отменённого
    решения, то есть ADR.
    """
