...
словарём draft-07. Это проверка словаря, а не валидация по метасхеме: последняя требует
валидатора JSON Schema, а такой зависимости в SPEC.md 2.3 нет.
"""
...
def check_draft_07(schema: dict[str, Any]) -> None:
    ...
    Не заменяет валидацию по метасхеме: та проверяет ещё и типы значений (что `required`
    — массив уникальных строк, что `pattern` — регулярное выражение). Для неё нужен
    валидатор JSON Schema, зависимости на который в SPEC.md 2.3 нет.
    """
