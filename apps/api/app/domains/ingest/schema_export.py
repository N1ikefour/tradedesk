...
SCHEMA_DESCRIPTION = (
    ...
    "она доезжает до Decimal вообще без float. "
    "ЧЕГО ЭТОТ ФАЙЛ НЕ ПРОВЕРЯЕТ. Тело, прошедшее валидацию по нему, всё ещё может "
    "получить отказ: пять правил сервера в draft-07 не выражаются, и вот они целиком. "
    f"(1) Батч не длиннее {MAX_DEALS_PER_BATCH} сделок, иначе HTTP 413. maxItems здесь нет "
    "намеренно: 413 про объём работы, а не про форму документа, и один документ не может "
    "быть одновременно кривым и слишком большим. "
    "(2) Границы numeric(18,2) и numeric(18,8) и запрет отрицательных объёмов и цен "
    "выражены только для числовой ветки: сравнить строку с числом JSON Schema не умеет, "
    "поэтому переполнение и минус в строке отсекает уже сервер. "
    "(3) time_msc и time_server описывают один момент и обязаны совпадать с точностью до "
    "секунды: сравнить два поля между собой draft-07 не может. "
    "(4) Шаблон time_server описывает форму, но не календарь: «2026-13-45T14:03:11» файл "
    "пропускает, сервер отвергает. "
    "(5) format: uuid в draft-07 — аннотация, и валидатор вправе её не проверять; "
    "account_id не в форме UUID сервер отвергает."
)
