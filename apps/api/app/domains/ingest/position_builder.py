...
class OpenPositionSnapshot:
    ...
    которого уже есть цена: на утверждении «в `net_pnl` открытой позиции нет плавающего
    результата» построена вся сводка `S2-05`, включая объяснение расхождения шапки
    журнала с колонкой. Плавающее число сделало бы это расхождение величиной, меняющейся
    от котировки к котировке.

    Инвариант именно такой, а не «в `net_pnl` открытой позиции только издержки»: у
    частично закрытой позиции там лежит **реализованный** результат уже закрытой части,
    и это факт брокера из `deals`, а не котировка. SPEC.md 7 считает `gross_pnl` как
    `Σ profit по deals`, поэтому у позиции без единого выхода остаются одни издержки
    (у сделки входа `profit = 0`), а у частично закрытой — издержки плюс то, что она уже
    заработала. Запись открытой позиции влияет **только** на `status`, и урезанный
    снимок делает это не обещанием, а свойством типа.
    """
